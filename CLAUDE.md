...
`session.ts` also self-initializes on every page load: if it finds `#userChipName` in the DOM (i.e., this is an internal/authenticated page), it paints the account menu (`#userChipToggle`/`#userChipMenu`: Ver tour, Reiniciar demo for Webmaster/Admin, Cerrar sesión), the sidebar (`#appSidebar`: unit links + a separate role-gated admin group with Usuarios/Importar) and the data-freshness indicator (`#dataStatus`) from `localStorage`, and redirects to `iniciar-sesion.html` if no demo session exists. Pages needing a *stronger* per-page role gate (e.g. `usuarios.html` is Webmaster-only) do an additional explicit check in their own script and redirect if the role doesn't match — `session.ts`'s auto-redirect only checks "is anyone logged in", not "is the right role logged in".
...
- **Internal/authenticated pages** (all require a demo session, all share the header pattern with `#sidebarToggle`/`#dataStatus`/`#userChipName` and an `.app-layout` = `#appSidebar` + `<main>`): `paneles.html` is the post-login **hub** — 3 clickable cards with animated SVG icons (Docencia, Investigación, Vinculación), the universal `DESTINO_LOGIN` target for all 5 roles, and the only page that runs the guided tour (`ts/tour.ts`, Driver.js v1.8.0 via CDN, once per role, replayable from "Ver tour"). Each unit has its own page (`docencia.html`, `investigacion.html`, `vinculacion.html`) driven by the shared engine `ts/unidad-dashboard.ts` (`initUnidadPagina`): 3 fixed categories per unit (Gestión X / X / Resumen general) listed in the sidebar with hash deep-linking (`#gestion`), each category holding `dashboards: VistaUnidad[]` (a dashboard-pill selector appears only when there is more than one), Chart.js via CDN, an 800ms simulated "Cargando dashboard…" state, and a floating context-aware assistant (`ts/asistente-flotante.ts`) that only appears when data has been imported; `usuarios.html` (Webmaster-only user management table with inline edit), `importar.html` (Webmaster/Admin only, page-level role gate; CSV import simulation — on success stores an ISO timestamp in `localStorage['panelacademico-datos-importados']`, which gates empty-state vs. populated dashboards and feeds the header freshness indicator; the form resets to a clean state afterwards; the page links a synthetic sample file, `test.csv` at the repo root, so the import can be tried without preparing data). Shared widgets: `ts/toast.ts` (top-right toasts), `ts/menu.ts` (accessible dropdown, also used by the account menu).

Role-based data scoping (e.g. Decanato only sees its own faculty + an aggregated institutional benchmark row, Visitante gets Exportar/Calculadora disabled) is enforced **only in the rendering logic** (`ts/unidad-dashboard.ts` → `filasParaRol`), not by hiding data at a real trust boundary — there is no backend yet to enforce it server-side. Don't treat this as real access control.
...
- `layout.css` — header/nav (both the public `.primary-nav` and the internal sidebar `.app-layout`/`.app-sidebar`/`.sidebar-*` and the account menu `.account-menu-*`), generic `.section`/`.section-head`, `.cta-section`, footer.
...
- `responsive.css` — both breakpoints (`768px`, `1024px`), imported last so its rules win the cascade. The rest of the site is min-width-only; the internal header's overflow risk on narrow phones (previously handled with a one-off `max-width:359px` hiding a text label) is now handled generically via `flex-wrap` on `.header-controls`/`.user-chip`-equivalent containers instead of a width-specific exception.
