...
(function initPaneles() {
    ...
    function formatearIndice(valor) {
    ...
    // de la API real.
    function calcularBenchmark(categoria) {
        const n = categoria.filas.length;
        const suma = categoria.filas.reduce((acc, f) => ({
    ...
    function filasVisibles(categoria) {
    ...
    function renderizarDashboard(categoria) {
    ...
    function pintarMensajes(contenedor) {
    ...
    const tabs = [
        ...PANELES.map((p) => ({ id: p.id, label: p.titulo })),
