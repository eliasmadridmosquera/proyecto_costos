...
(function initRecuperarForm() {
    ...
    if (!(elCorreo instanceof HTMLInputElement))
        return;
    const input = elCorreo;
    function mostrarError(mensaje, animar = false) {
        const errorEl = document.getElementById('correo-error');
        const wrapper = input.closest('.form-field');
        if (!errorEl || !(wrapper instanceof HTMLElement))
            return;
    ...
    const revalidar = () => mostrarError(validarCorreoInstitucional(input.value));
    ...
    form.addEventListener('submit', (evento) => {
        ...
        mostrarEstado('Si el correo está registrado, recibirás instrucciones para restablecer tu contraseña.', 'success');
        form.reset();
        mostrarError(null);
    });
})();
