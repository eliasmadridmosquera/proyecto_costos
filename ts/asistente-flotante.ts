...
(function initAsistenteFlotante(): void {
  if (!hayDatosImportados()) return;
  ...
  function responderPregunta(pregunta: string): string {
    if (!contextoAsistente) return 'Todavía no hay una vista activa para consultar.';
