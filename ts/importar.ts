...
function parsearCsv(texto: string): { columnas: string[]; filas: string[][]; totalFilas: number } {
  const lineas = texto.split(/\r\n|\n/).filter((l) => l.trim().length > 0);
  if (lineas.length === 0) return { columnas: [], filas: [], totalFilas: 0 };
...
(function initImportador(): void {
  ...
  if (
    !(elDropzone instanceof HTMLElement) ||
  ...
  const dropzone: HTMLElement = elDropzone;
  const fileInput: HTMLInputElement = elFileInput;
  const anioSelect: HTMLSelectElement = elAnio;
  const categoriaSelect: HTMLSelectElement = elCategoria;
  const submitBtn: HTMLButtonElement = elSubmit;
  const previewBox: HTMLElement = elPreviewBox;
  const previewFilename: HTMLElement = elPreviewFilename;
  ...
  function mostrarError(campo: 'anio' | 'categoria' | 'archivo', mensaje: string | null): void {
  ...
  function actualizarBotonEnvio(): void {
    submitBtn.disabled = archivoActual === null;
  }

  function renderizarVistaPrevia(archivo: ArchivoParseado): void {
    ...
    if (thead) thead.innerHTML = `<tr>${archivo.columnas.map((c) => `<th>${c}</th>`).join('')}</tr>`;
    if (tbody) {
  ...
  function procesarArchivo(file: File): void {
  ...
  form.addEventListener('submit', (e: SubmitEvent) => {
    ...
    window.setTimeout(() => {
      ...
      crearToast(
        'toastContainer',
        'success',
        `Se importaron ${filasImportadas} filas. Métricas recalculadas para ${categoriaLabel} — ${anioSelect.value}.`
      );
