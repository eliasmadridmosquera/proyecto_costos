...
interface EnlaceInterno {
  href: string;
  label: string;
  /** Si está presente, se pinta un <ul class="sidebar-sublist"> vacío con este
   * id (sidebarSub-{subId}) — la propia página de unidad lo rellena con sus
   * categorías (ver ts/unidad-dashboard.ts). */
  subId?: string;
}
...
function leerFechaImportacion(): string | null {
  try {
...
function formatearFechaImportacion(iso: string): string {
...
function actualizarEstadoDatos(rol: RolDemo): void {
...
function tourYaVisto(rol: RolDemo): boolean {
  try {
...
function marcarTourVisto(rol: RolDemo): void {
  try {
...
function limpiarTourVisto(): void {
  try {
...
const ENLACES_UTILIDAD: Record<RolDemo, EnlaceInterno[]> = {
  webmaster: [
...
(function initHeaderInterno(): void {
  ...
  const sinHtml = (ruta: string): string => ruta.replace(/\.html$/, '');
  const paginaActual = sinHtml(window.location.pathname.split('/').pop() ?? '');

  function renderEnlace(item: EnlaceInterno): string {
  ...
  if (sidebar instanceof HTMLElement && elToggle instanceof HTMLElement) {
    ...
    const sidebarToggle: HTMLElement = elToggle;

    function setSidebar(abierto: boolean): void {
  ...
  if (menuLista instanceof HTMLElement) {
    ...
    const listaCuenta: HTMLElement = menuLista;
