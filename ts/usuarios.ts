(function initUsuarios(): void {
  ...
  if (!(elTbody instanceof HTMLTableSectionElement)) return;
  ...
  const tbody: HTMLTableSectionElement = elTbody;

  const usuarios: UsuarioSistema[] = USUARIOS_INICIALES.map((u) => ({ ...u }));
  let editandoId: number | null = null;

  function mostrarEstado(mensaje: string): void {
    if (!status) return;
  ...
  function esRolAsignable(valor: string): valor is RolAsignado {
    return (ROLES_ASIGNABLES as string[]).includes(valor);
  }

  function opcionesRol(rolActual: RolAsignado): string {
    return ROLES_ASIGNABLES.map((r) => `<option value="${r}"${r === rolActual ? ' selected' : ''}>${r}</option>`).join(
      ''
    );
  }

  function opcionesFacultad(facultadActual: string | null): string {
    const vacia = `<option value=""${facultadActual ? '' : ' selected'}>—</option>`;
    const resto = FACULTADES_DEMO.map(
      (f) => `<option value="${f}"${f === facultadActual ? ' selected' : ''}>${f}</option>`
    ).join('');
    return vacia + resto;
  }

  function filaEdicion(u: UsuarioSistema): string {
    return `
  ...
  function filaNormal(u: UsuarioSistema): string {
    return `
      <tr data-id="${u.id}">
        <td>${u.nombre}</td>
        <td>${u.correo}</td>
    ...
    tbody.querySelectorAll('tr').forEach((tr) => {
      ...
      if (selectRol instanceof HTMLSelectElement && selectFacultad instanceof HTMLSelectElement) {
        selectRol.addEventListener('change', () => {
          ...
          if (!esDecanato) selectFacultad.value = '';
        });
      }
      ...
      tr.querySelector('.btn-guardar')?.addEventListener('click', () => {
        ...
        if (!esRolAsignable(selectRol.value)) return;
